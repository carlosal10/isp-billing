  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <select value={plan} onChange={(e) => setPlan(e.target.value)} required>
        ...
        {plans.map((p) => (
          <option key={p._id} value={p._id}>{p.name} - {p.speed}Mbps - {p.price} KES</option>
        ))}
      </select>
        ...
        <>
          ...
          {networkType === "static" && (
            <div className="static-config">
              <input value={staticConfig.ip} onChange={(e) => setStaticConfig({ ...staticConfig, ip: e.target.value })} placeholder="IP Address" required />
              <input value={staticConfig.gateway} onChange={(e) => setStaticConfig({ ...staticConfig, gateway: e.target.value })} placeholder="Gateway" required />
              <input value={staticConfig.dns} onChange={(e) => setStaticConfig({ ...staticConfig, dns: e.target.value })} placeholder="DNS" required />
            </div>
          )}
        </>
      )}
      ...
      <button type="submit" disabled={loading}>
        {type === "Add" ? <MdAdd className="inline-icon" /> : type === "Update" ? <AiOutlineEdit className="inline-icon" /> : <RiDeleteBinLine className="inline-icon" />} {type} Customer
      </button>
    </form>
  );
}
